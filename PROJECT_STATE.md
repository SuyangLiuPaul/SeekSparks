...
Last updated: 2026-08-31 (twenty-fourth entry)
...
| `main` | **fce19c7** — #315 residue, second pass: closed the last deferred field. `word_study_style.dart`'s dense-column `translit`/`micro` resolved to 9.0px at the default Menu Size (`type.chrome - 2`, and `chrome` IS `WbMetrics.smallPrintFloor`), a fourth shape neither ratchet regex can see — a named-argument initialiser on a data class, read six files away in `originals_sheet.dart` as `fontSize: _st.translit`. Changed both to `type.chrome`. The deferral's stated cost (four ranks collapsing to one size) was checked at all six call sites and does not hold: every one already carries rank on a non-size channel (colour, italics, weight, tracking); one of the six (`:1531`) is the CBOL CC-BY-NC-SA 4.0 attribution the licence obliges the app to print. The phone-modal column stays deliberately under the floor (#284's pinned decision) — a new test now asserts that breach out loud instead of leaving it silent. New 4-test group in `word_study_style_test.dart` reads resolved values (the only instrument that can see this shape); confirmed 3 of 4 red before the production edit. Amended the ratchet's `finished` list comment so "zero literals" no longer implies "nothing under the floor". +4 tests, 3,751 green, analyze exit 0. NOT deployed — counter 2 of 3. No screenshot taken; whether a caption reads as subordinate at equal size to its heading is now unverified across a *wider* set (four ranks, not two) than the first pass left it. |
| `main` (previous) | **f3cac03** — #315 residue, landed the killed prior run's small-print-floor repair: all 52 `fontSize: t.chrome - N` sites (15 files) raised to `t.chrome` = `WbMetrics.smallPrintFloor`, plus a new ratchet forbidding the notation with an 18-entry allowlist for above-floor additive offsets. The `lib/` edit was already correct on disk and uncommitted; this run audited it (52 removed lines, 52 added, each a mechanical `<recv>.chrome - N` → `<recv>.chrome`), then re-baselined `places_pane_test.dart`'s "the added line does not eat the map" on three measured canvas heights (293.0 pre-footer, 276.0 at `t.chrome - 1`, 267.0 at `t.chrome` — a 9px cost, no wrap, no overflow). +1 test, 3,747 green, analyze exit 0. NOT deployed — counter 1 of 3. |
| `main` (before that) | **230f91e** — #317: the ruler's tooltip cited `kBandOnFoot`'s fixed sentence regardless of which band the days above it were computed at; it now calls the already-shipped `travelBandBasis()` for the reader's own band and appends `travelBandNotOurs()` on a non-default choice, matching what the journey panel already did. Added one visible footer line for the non-default case (`travelBandLabel()`, key `places-map-band`) — the band picker lives in the journey panel, which can be closed while `travelBand` still reaches the map, and a tooltip is a hover, unreachable on a tablet (#312). +4 tests (19 in `places_pane_test.dart`), 3,746 green, analyze exit 0. Deployed to dev v1.6.186 — counter 3 of 3. Cut `1569a8a`. |
| `main` (earlier) | **841c0d4** — #317's own row named the one item taken knowingly unpaid: BibleWorks' Travel Speed Window (bwh33) lets a reader set the speed, ours was one hardcoded band. A free-form box would have turned a cited ORBIS figure into an uncited one, so the choice offered is between ORBIS's own three adjacent daily-distance pairs (12–20 / 20–30 / 30–36 km/day) instead — every number on screen stays a quotation. +7 tests (29 in the file), 3,742 green, analyze exit 0. NOT deployed — counter 2 of 3. |
| `main` (older) | **70e7912** — closed the #317 join gap (see `pubspec`/dev row above) and shipped v1.6.184 to dev. |
| deploy | **Skipped this iteration — counter 2 of 3.** Last deploy was `1569a8a` (v1.6.186, counter 3 of 3). Dev is 2 commits / 2 iterations behind main (`f3cac03`, `fce19c7`). `./tools/release_web.sh` not run; no `pubspec.yaml` bump. |
| Suite | **Full run: 3,751 passed, 0 failed** (+4 for the new word-study-style-floor group; the prior 3,747 baseline otherwise unchanged). `flutter analyze` exit 0. |
| CI | Not re-checked this run; `f3cac03` was `in_progress` when last written, the two runs before it both `success`. |
