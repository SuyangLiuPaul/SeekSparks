...
| | |
|---|---|
| `pubspec` / dev | **1.6.158** — #315's `settings_page.dart` slice, built and deployed 2026-08-24 from a **detached worktree at the pushed SHA** (e7268da). `release_web.sh` derives its project root from its own path, so a worktree copy just works; the bump lands there and is ported back by hand. **But the dev ALIAS does not serve it.** 83 seconds after this deploy went live, a **second writer published a `v1.6.157` build over it** (deploy `6a8b9178…` at 00:34:00 vs `6a8b9125…` at 00:32:37), so `seeksparks-dev.netlify.app` reports 1.6.157 and only the immutable URL `6a8b91259dd71dcee800c5ee--seeksparks-dev` carries the 1.6.158 label. **The fix is live anyway and this was verified, not assumed** — their build came off the shared working tree *after* e7268da was committed to it, so it contains the change; driving the real Font Size slider to 40 pt on the live alias shows the section headers, the profile subtitle and the card small print all roughly doubled. **Deliberately not re-published over them**: it would clobber a deploy they may be verifying, and buys no behaviour that is not already there. Note their bundle was built from a *dirty* tree (`radial_chronology_page.dart`, `url_sync_service*`, `main.dart` all uncommitted) and so matches no commit |
| prod (seeksparks.netlify.app) | **1.6.136** — 21 versions behind, by design: prod ships only on the owner's word |
| `main` | **e7268da** — #315: the 28 saturating ceilings and 5 literals on the page that *carries* the Font Size slider. Deployed (see above); the undeployed counter resets |
| Suite | **3,312 tests**, green; `flutter analyze` exit 0 |
