...
| | |
|---|---|
| `pubspec` / dev | **1.6.173** — #315's ELEVENTH pass: 37 sites whose design size was already under the app's own small-print floor. Built and deployed dev 2026-08-25 **from the working tree**, `git status --porcelain` showing only the second writer's untracked `output/`, i.e. every tracked file matched the pushed SHA `efac8ee`. **Alias and immutable URL both serve `1.6.173`** (`version.json`, cache-busted on each) and the in-app status bar reads `v1.6.173 · 2026-08-25 05:43 UTC+10`. Photographed on the deployed build at **both** the default and 12 pt: the chronology axis strip draws its ticks and epoch names without collision at either, and at the default the axis (11 px chrome) and the lane names (12 px body) are consistent — the raise moved the strip with it, because `_axisHeight` measures the strip from the axis font. Deployed although the undeployed counter stood at 0, because #315 is **human-reported** and the standing rule exempts those. **Unexplained and recorded rather than claimed:** two brand-new headless Chrome profiles both booted with `flutter.fontSize = "12"`, which is why the first photograph came back at the slider minimum; clearing `localStorage` and reloading the same origin wrote the correct `fontSize: 20`, so **the app does not ship a 12 pt default** — that was checked before anything was written down. Worth one probe next iteration; it is not a defect on the evidence available. Before it, **1.6.172** — #315's TENTH pass: the reader's own `Aa` sheet could not reach the app's maximum. Built and deployed dev 2026-08-25 **from the working tree, not a detached worktree** — a deliberate reversal of the last two builds' method, and the reason is worth keeping: four asset paths the app loads are **gitignored** (`assets/nasb-ev.json`, `assets/nsn-plus.json`, `assets/build/`, `assets/tagged/nsn-plus/`), so a worktree build would have shipped **silently without them**. `git status --porcelain` showed only the second writer's untracked `output/`, i.e. every tracked file already matched the pushed SHA `585e7ee` exactly, so the worktree bought nothing and risked a data-less bundle. Deployed although the undeployed counter stood at 0, because #315 is **human-reported** and the standing rule exempts those. Before it, **1.6.171** — #315's ninth pass, second build: the alphabet cell widened from `math.max(44, name*4.0)` to `math.max(48, name*4.4)`. Built and deployed 2026-08-25 from a **detached worktree at the pushed SHA** (`54e71f4`), the shared tree carrying the second writer's untracked `output/`. **Alias and immutable URL both serve `1.6.171`** (`version.json`, cache-busted on each), and the in-app status bar reads `v1.6.171 · 2026-08-25 03:59 UTC+10`. **Re-photographed at 12 pt: all 22 romanised names now sit on one line** — "Lamed" and "Samek", which wrapped on 1.6.170, no longer do, and every cell is the same height. This build exists *because* of the previous one: the regression it fixes was introduced by 1.6.170 and is invisible to the suite, so only a photograph of a deployed build could close the loop. Before it, **1.6.170** — #315's ninth pass proper (`bbba27f`): 23 frozen sites in `bible_trivia_page.dart` routed through `WbType`. Built and deployed 2026-08-25 from a detached worktree at the pushed SHA; alias and immutable URL both served it, status bar `v1.6.170 · 2026-08-25 03:35 UTC+10`. Photographing it at 12 pt is what exposed the wrap. Both deployed although the undeployed counter stood at 0, because #315 is a **human-reported** defect and the standing rule exempts those from the counter. Before it, **1.6.169** — #315's eighth pass: the ceiling that put the artefact's name under its own description. Built and deployed 2026-08-25 from the shared tree (`release_web.sh` bumped `pubspec` itself; the bump was committed after the deploy printed the number, as `99e0f86`), the second writer's untracked `output/` left alone. **The status bar on the deployed alias reads `v1.6.169 · 2026-08-25 02:09 UTC+10`**, photographed in headless Chrome after a cache-busting navigation. Deployed although this was only undeployed loop iteration 1 of 3, because #315 is a **human-reported** defect and the standing rule exempts those from the counter. Before it, **1.6.168** — #315's seventh pass: the frozen sizes inside the two files the ratchet calls `finished`. Built and deployed 2026-08-24 from a **detached worktree at the pushed SHA** (`d18018b`), the shared tree carrying the second writer's untracked `output/`. **Alias and immutable URL both serve `1.6.168`** (`version.json`, cache-busted on each). Deployed although this was only undeployed loop iteration 2 of 3, because #315 is a **human-reported** defect and the standing rule exempts those from the counter. Before it, **1.6.167** — #315's sixth pass: the book label that could not grow past its tile. Built and deployed 2026-08-24 from a **detached worktree at the pushed SHA** (`37c4bb2`), the shared tree carrying the second writer's untracked `output/`. **The alias serves it** — `version.json` reads `1.6.167`, cache-busted. Before it, **1.6.166** — #315's fifth pass: the sermon title that shrank below the sermon. Built and deployed 2026-08-24 from a **detached worktree at the pushed SHA** (`ab7eef7`), the shared tree carrying the second writer's untracked `output/`. **Alias and immutable URL both serve `1.6.166`** (`version.json`, cache-busted on each). Before it, **1.6.165** — #295's grammar audit: three wrong search counts fixed. Built and deployed 2026-08-24 from a **detached worktree at the pushed SHA** (`9384762`), the shared tree carrying the second writer's untracked `output/`. Alias serves it (`version.json` reads 1.6.165), and the deployed `main.dart.js` carries the new `The word gap after` refusal — checked against a control string known to ship, since a bare absence in that bundle proves nothing. Before it, **1.6.164** — released by the **second writer**, not the loop: branding/launcher-icon fixes and the splash download bar (`983c0ea`…`200cfdf`, 13:27–14:22). Its build ran against the shared working tree and so also carries the loop's then-uncommitted #317 work; see the `main` row. Before it, **1.6.161** — #312 item 7: the four phrasing level chips now carry the line count they would draw on this passage, and the chosen one says what it cuts at. Built and deployed 2026-08-24 from a **detached worktree at the pushed SHA** (`ccfafc1`), the second writer having released 1.6.160 from the shared tree twenty minutes earlier. Alias and immutable URL both serve `1.6.161` (`version.json`, cache-busted). Previously: **1.6.159** — #315's fourth mechanism: `workbenchTheme`, the app's only theme, sized every Material role from constants. Built and deployed 2026-08-24 from a **detached worktree at the pushed SHA** (`01b5c80`), because the second writer's 106 modified launcher icons and an untracked `output/` were live in the shared tree and `release_web.sh` builds from the *working tree*. **The alias serves it this time** — unlike v1.6.158, which a second writer's build overwrote 83 seconds later; `version.json` and the in-app status bar both read 1.6.159, checked after a second cache-busting navigation to defeat the service worker |
| prod (seeksparks.netlify.app) | **1.6.136** — 21 versions behind, by design: prod ships only on the owner's word |
| `main` | **cf914ff** — #318 phase 7: **the flood is dated twice and the parser could not read the second verse.** The chart placed the flood from Genesis 7:6 ("Noah was six hundred years old") and never mentioned **Genesis 7:11**, which dates the same day to "the six hundredth year of his life" — an ordinal, 599 complete. The two are **one year apart**, and because Shem's birth is counted off the flood the choice moves **every year on the axis after AM 1656 / 2242**. The chart already makes that subtraction at 1 Kings 6:1 and states it; here it made the opposite choice silently. **It was silent because `en_ordinal_runs("six hundredth year")` returned 106** — `EN_ORDINALS` listed `"hundredth"` as a plain 100 and `_value` **added** it. Unreachable, because the ordinal reader's only address was 1 Kings 6:1, which spells the multiplier the ordinary way; so nothing was wrong on screen and no test could see it. `"hundredth"` is now a multiplier (still 100 alone); 1 Kings 6:1 still reads 480. **No figure moved** — 21 insertions, 1 deletion in the asset. What ships is a measured per-tradition note carrying **both** AM years (1656/1655, 2242/2241) in three locales, guarded by a generator `SystemExit` if the two verses stop naming the same year — verified to fire. **`ChronologyEpoch.noteFor` is never called anywhere in `lib/`:** all five epochs carry sourcing no reader has seen and the flood's English was **wrong** (7:11's wording on 7:6's verse), so the disclosure was routed to a `ChronologyNote(personId: 'noah')` and pinned by a **widget** test. All three new tests fail against the pre-change asset. Before it, **ebec932** — #318 phase 6: the chronology past Moses is counted, not drawn. |
| Suite | **3,464 tests**, green; `flutter analyze` exit 0 |
...
| #318 | Interactive Bible chronology, featured module | open — phases 1–7 shipped. Bars run to the death of Moses; the exodus→temple era is **counted below the chart, never drawn** (MT 530 vs 479, LXX 520 vs 439). Phase 7 disclosed the one-year choice at the flood (Genesis 7:6's cardinal over 7:11's ordinal) that moves every year on the axis after it. **Next slice, already scoped:** the five epoch `note` paragraphs are in the asset and `ChronologyEpoch.noteFor` is never called — render them somewhere that is not the header, which is an unconstrained `Column` above an `Expanded` chart |
