...
| CI | **green — `success` on `30afd82` (the tip) and on `82a6cee`, read back from `gh run list` after both runs completed**, not inferred from the local macOS suite. The local run is the VM on macOS; CI runs it again on Linux, and this iteration's new page tests warm `FamilyTreeService` through `rootBundle` in `setUpAll`, which is exactly the kind of thing that can pass on one host and not the other. |
