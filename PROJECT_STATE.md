...
| `main` | **a48f518** — the reading pane's own headings, disclosed: `assets/section_titles.json`'s 1,443 app-written headings per title set (all three sets) each carry an app-written background blurb, rendered by default in the reading pane in the register a printed edition's own 段落標題 occupies, and nothing parsed the asset's `_meta` saying they are original to the app and not from any published edition. Third instance of the class #318 phase 24 and #292 each fixed once. Trilingual `note` added to `_meta` (insertion only, byte-identical otherwise); new `SectionTitleMeta`/`provenanceNote()` in `section_title_service.dart`; one line inside the reading pane's already-collapsed context panel; an About-page attribution row; extended settings subtitle. Deliberately deferred, same unparsed-header shape, measured this run: `book_introductions.json`, `bible_evidence.json`, `daily_verses.json`, `gospel_synopsis.json`, `chronology.json`, `bible_timeline.json`, `family_tree.json`, `bible_journeys.json` — each needs its own surface, not a sweep. +5 tests. Full suite 3,823 green (was 3,818), analyze exit 0. Deployed to dev at counter 3 of 3, `18df2d6` bumped to v1.6.195. |
...
| deploy | **Skipped this iteration — plan-mandated counter 2 of 3.** Last deploy was `db79c5e` (v1.6.194, prod+dev, 2026-08-31, by a concurrent session); dev is now 2 commits / 2 iterations behind (`b2a2c50`, `47c7c1c`). `./tools/release_web.sh` not run; no `pubspec.yaml` bump. |
| Suite | **Full run: 3,818 passed, 0 failed.** `flutter analyze` exit 0, run last after the final edit. |
