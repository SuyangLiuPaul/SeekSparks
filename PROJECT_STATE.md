...
| CI | **green — `success` on `e202642` (the tip) and on `53f8fd3`, read back from `gh run list` after both runs completed**, not inferred from the local macOS suite. The distinction has teeth this iteration: the new coverage test reads `lib/` source files off disk by relative path, which depends on the working directory CI runs `flutter test` from, and the four new search guards warm `FamilyTreeService` through `rootBundle` — both are the kind of thing that passes on one host and not the other. **The local run is the VM on macOS; the app ships as JS**, and nothing here does arithmetic near an integer boundary, so no `--platform chrome` re-probe was warranted. |
