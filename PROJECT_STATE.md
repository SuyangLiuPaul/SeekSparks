...
Last updated: 2026-08-26 (nineteenth entry)
...
| | |
|---|---|
| `pubspec` / dev | **1.6.183** — #318 phase 20, the timeline's 88 person links rendered and the Moses disagreement repaired. Cut at `82a6cee`, deployed because the counter stood at **3 of 3**, and **photographed rather than assumed**: `moses_born` prints **1526 BC** unhedged with **Dated by** Exodus 7:7 / 1 Kings 6:1 above **People** Moses / Jochebed / Amram, and tapping the Moses chip opens the sheet reading **1526 BC – 1406 BC**, so the repair is visible to a reader and not only to a test. `jesus_born` shows Jesus / Mary / Joseph (husband of Mary) as neutral grey chips above the blue reference chips, both rows square. The deployed `assets/family_tree.json` was read back off the immutable URL as well: `moses` is −1526/−1406 and both `corrections` entries are present. |
| `pubspec` / dev (previous) | **1.6.182** — #318 phase 17, the Bible's own story merged onto the wheel, plus the `localizedReferenceLabel` extent fix. Cut at `9c930fd`, and **photographed rather than assumed**: the splash reads `v1.6.182`, the hub reads **22 · 62 · 588**, the 1010–931 BC cluster lists David, the Davidic Covenant, Solomon, the Temple and the division alongside the wheel's own Hiram of Tyre and the Bantu expansion, *David Becomes King* prints "year from Thiele's chronology of the kings" (an arm that was a **dead branch** before this merge, so it was a latent claim and not a shipped defect), and Noah's Flood prints **`Genesis 6-9`** / **`创世纪 6-9`** where before the fix it printed `Genesis 6`. |
| `pubspec` / dev (older) | **1.6.181** — the three defects the *photograph* of v1.6.180 found (see `HANDOFF.md`). Cut at `bfd1ccf`; `version.json` verified on both the alias and the immutable URL, and the find feature re-driven on that build — "Magna Carta" → **1 result** (not "1 results"), the sheet sized to its one row, the English hint no longer advertising a Chinese keyboard. |
| prod (seeksparks.netlify.app) | **1.6.136** — dev is at 1.6.183; prod ships only on the owner's word. *(The "21 versions behind" this row carried for weeks was never derivable from either number and is withdrawn rather than replaced with another guess.)* |
| `main` | **b188a0d** — **the Atlas estimated travel time where it knew least and refused where it knew most.** The ruler footer printed one confident figure, `ceil(km / 32)` captioned "about", between two arbitrary gazetteer points; the journey overlays — where **every leg carries the mode of travel the text itself uses** — printed no time at all. **The refuter killed my first framing and that is the durable finding: 32 is not unsourced folklore.** It is ISBE's "day's journey" (32–40 km, derived from a pack mule at 3 mph × 8 h), so this was never *replace an unsourced number with a sourced one* — it was **a choice between two published conventions, and it needed a reason**. **Deuteronomy 1:2 supplied one, and it is the kind of argument this app should prefer over an argument from method:** eleven days from Horeb to Kadesh-barnea, **244.3 km** measured through our own `haversineKm` over our own gazetteer (Horeb = Sinai = 28.53/33.97, Kadesh-barnea = 30.68/34.49). **ISBE gives 7–8 days and contradicts the verse; ORBIS's 20–30 km/day gives 9–13 and contains it.** The figure we shipped would have answered "about 8 days" for a stretch the text calls eleven. **Both arithmetics are pinned** in `test/travel_time_test.dart` so reverting to a faster speed is a visible act rather than a plausible tidy-up. New `lib/utils/travel_time.dart` holds the band — both ends quoted from **one sentence** of ORBIS v1.0 (Scheidel, Meeks and Weiland, Stanford, May 2012), and it is **a band of LOAD, not of pace**: 30 is a party on the march, 20 a party carrying its own baggage — plus `travelOf`, which splits a route's **drawn segments** by leg so the buckets always sum to the total the panel already prints. Land legs get a band with three hedges that must never be edited away: the source, the straight-line floor, and the unit (**days on the road, not elapsed time** — Acts 18:11 has Paul eighteen months at Corinth, Acts 20:31 three years at Ephesus). **Sea legs get no number and say why, and the reason had to be re-based too.** My draft said no defensible sailing speed exists; that is false and falsifiable — Casson gives 4–6 knots downwind, and Alexandria–Rome passages are attested at 18–25 days. The claim that survives is narrower: **no speed can honestly be applied to a CHORD**, which does not carry the wind's bearing, and a square-rigger could not sail our straight line anyway. **Acts 27 is the proof in our own data** — the ship runs under the lee of Cyprus and again under Crete, which is not the shortest way and is why there was a voyage to write about. Two figures in my draft were simply wrong and are gone: "1–2 knots against" (modern rig studies put speed made good to windward under ~2.5) and "more than an order of magnitude" (6 vs 1.5 is 4×). **Every non-zero bucket prints with no threshold**, because `paul-rome` is **93.6% sea** (2,803 of 2,996 km) and `jesus-mark` **58% unstated** — a band shown for paul-rome's three land legs with nothing beside it would tell a reader Paul walked to Rome in a week and a half. **A per-journey caution belongs in that journey's provenance, not in code:** ORBIS models the Roman world c. 200 CE and its marching-party figure understates Numbers 33, a nation moving with its flocks (Gen 33:13–14 has Jacob set his own pace by the young of the flock) — so one sentence went into `exodus-wilderness`'s `basis` in all three languages rather than a special case in `travel_time.dart` or a third speed constant invented to cover it. `daysOnFootFor` and `placesMapDays` are **deleted outright**, and the refuter's grep found the two test files that would have broken. **Three caveats now ride beside the Deut 1:2 guard**, one of which is load-bearing: the check **presupposes a southern or Arabian Sinai** — Jabal al-Lawz is near enough equidistant (~239 km) to leave the arithmetic alone, but Jebel Halal (~44 km from Kadesh) or Har Karkom (~50 km) would not shift the check, they would **collapse** it. Also recorded and left alone: our Kadesh-barnea sits ~7 km from Tell el-Qudeirat's published coordinates, which is a gazetteer question, not a travel-time one. **+23 tests, 3,717 green, analyze exit 0.** The two widget guards render **zh-Hans**, so they can see the notes reach the screen (`步行 55–82 天`, `水路 2803 公里`, `水路不作天数估算`, `牛羊同行`) but are blind to English plurals by construction — which is why the singular/plural rule is asserted **against the string table** instead, where `1 day on foot` can actually fail. |
| `main` (previous) | **53f8fd3** — **the wheel could not find people its own records name, and the reason is a defect this merge has now shipped three times.** `bibleNarrativeEvents` converts `TimelineEvent` → `WheelHistoryEvent`; a field the target class has no constructor parameter for is dropped **at the call**, silently, and **nothing in the repository can see it** — the asset tests pass because the data is present, the timeline page's tests pass because that page reads `TimelineEvent` directly, the wheel's tests pass because they assert about the fields it *does* carry. It went `basis` (phase 17), `datingRefs`/`septuagintYear`/`era` (phase 19), `personIds` here. Measured through the app's own engine over the merged **754 records** (588 events / 82 nations / 62 powers / 22 streams): **61 events carry people, 88 links, 37 distinct, all resolving**, and **five names returned zero hits in all three scripts** — the search box saying "no results" about records it holds. **The claim is hedged rather than dropped on one of the five.** Four — Aaron, Amram, Jochebed, Miriam — are in no title and no description anywhere on the wheel: true false absences. The fifth, **Jeconiah, the wheel does carry**, spelled **Jehoiachin / 约雅斤** on `exile_of_jehoiachin` and `evil_merodach`. He was unreachable only under the name the *family tree* prefers, so this unifies two spellings onto one subject rather than restoring a missing one. **The ratchet is the durable part:** `test/wheel_timeline_field_coverage_test.dart` reads both class declarations out of the **source** and fails when a declared field is not read by the merge — run against the two commits that shipped the defect it reports `personIds` at `30ff8d8`, and `personIds, datingRefs, septuagintYear` before phase 19. **Its measured limit is written into it:** it does **not** report phase 19's fourth field, `era`, which was read to choose a stream while its value went nowhere — *"mentions the field" cannot tell reading from carrying* — which is why a second test pins the pass-through set separately. Two traps in building it, both hit and both recorded: a function with **named parameters** has a `{ … }` of its own, so brace-matching from the first brace after the signature returns the parameter list; and the pre-fix function was an **expression body**, where the first brace is a `${e.id}` interpolation — a red for entirely the wrong reason that **would have been logged as a successful differential on the strength of the exit code alone**. Names are resolved once at merge and **copied**, never looked up while painting: `FamilyTreeService.byId` is synchronous and answers null until its own load finishes, so a chip resolving itself mid-paint would render empty for a frame and *the search beside it would agree*, reporting an absence that was really a race. The function stays pure by taking `people` as a parameter; the service does the awaiting. Search ranks the person tier **above `reference`** (14 → 15) — a person is a subject of the record, an address is only where to find it — and exactly one record's linked name also matches its own reference (`bible:ruth`), whose title claims it first, so **no reference row reclassifies**. **The refuter overturned two things.** Twelve family-tree ids are **also wheel record ids** — `noah, shem, ham, japheth, arphaxad, eber, peleg, reu, serug, terah, haran` are *nations* and `judah` is a *stream*, because both datasets name a people after the man it descends from; only `noah` is currently linked, so the live surface is one id, but the overlap is structural and is now pinned. And the wheel is **catching up to the linear timeline page**, which has rendered person chips and searched them since it shipped — not doing something new. The row is **plain underlined text**, not a copy of `_RefChip`, which would have carried `Radius.circular(4)` onto a page whose only two radius call sites are `BorderRadius.zero`; and not `wb.link`, because on this page that colour means the tap *leaves* for a verse. **+13 tests, 3,694 green, analyze exit 0.** The four search guards were sized by **severing the merge's `people` argument** rather than against the pre-fix commit, which cannot compile them — exactly four assertions red, all in `wheel_search_test.dart`, with the data-layer group correctly staying green because it passes `people:` explicitly and so measures the pure function rather than the service. **The tap test was proved non-vacuous by injecting a single `WbType.of(context)` into `_showPerson`**, which reds it on provider's `debugBuilding` assertion — the bug class that once meant no detail sheet on this page could open at all in a debug build, and that `flutter analyze` and a release build are both blind to. |
| `main` (before that) | **30afd82** — **the chronology and the family tree name the same people and had never compared notes.** `bible_timeline.json` carries `personIds` on **61 of 98 events, 88 links naming 37 people**, and the field appeared at **exactly three lines in all of `lib/`, all three inside the model**. Nothing rendered it, so nothing checked it — **a field with no call site is a field with no audit**. They disagreed: `family_tree` dated Moses −1525..−1405 against the timeline's **−1526** and **−1406**, both files stating the year EXACTLY, on the same `scripture+thiele` basis, citing the same two verses. Not two reconstructions — **one derivation written twice with one copy contradicting its own arithmetic**; Isaac (−2066) and Jacob (−2006) agree exactly across both files and are the controls that prove the offset was isolated. **The decision rule is the durable part:** both sides exact → repair; **either** side hedged → name it and leave it, because picking a side would be one test file deciding a chronology. So **Jesus** (tree c. 4 BC – c. AD 30, timeline −5 and AD 33, both `conventional` and approximate) is allowlisted with its reason, and **Ishmael** — −2080 in *both* files but stated exactly by one and hedged by the other, **one fact at two confidences** — is named and not repaired, because the repair STRENGTHENS a published claim. `test/timeline_person_join_test.dart` **walks** the join instead of enumerating it (unlike `cross_asset_year_agreement_test.dart`, this pair shares a key), with floors of 88/61/37 so an emptied field cannot make the file pass by checking nothing. Two detectors: **id-derived exactness** and **lifespan containment**. Against the pre-fix asset the first went red on **both** Moses fields and the second on **only the birth** — a death dated a year EARLY still falls inside the lifespan, so containment was structurally blind to `moses_dies`. Containment is a **list with written reasons, not a bare assertion**, because the Transfiguration puts Moses in an AD 30 scene on purpose and `wilderness_40` is keyed to its END so Aaron is correctly linked one year after his death. A third test guards the **namespace**: two Manassehs and two Eleazars are in the tree, and **a link that resolves is not a link that resolves CORRECTLY** — no id check can see a chip reading "Manasseh" that opens the wrong one. The **35 Anno Mundi people are out of reach by unit, not by judgement**. The render exposed a worse defect than it fixed: **耶哥尼雅 appears in no event text anywhere in the asset**, so the page said "no results" about a record it holds — linked names in all three scripts now join the haystack. And **a copied widget carried a rule violation with it**: `_PersonChip` was copied from `_RefChip` and arrived holding two rounded corners, taking the file from its declared **5** #279 offences to **7** and producing the run's only red, in neither file this phase wrote. Both chip species were squared rather than the growth declared — budget **5 → 3**. |
| deploy | **SKIPPED, deliberately — this is iteration 2 of 3 since v1.6.183.** The counter is not reset: phase 21 skipped at 1 of 3 and this is the second. **The judgement call, stated because it is genuinely ambiguous:** #317 *is* a human-reported ticket, which would trigger the deploy-on-demand clause — but the specific defect fixed here (a travel figure contradicting Deuteronomy 1:2) was found by this loop, not reported by a human, so the clause reads as **not** triggered and the conservative option is to hold. Prod is untouched by this either way. **What is undeployed: 2 commits, 2 iterations.** **The next iteration is 3 of 3 and MUST deploy**, at which point **two photographs are owed**: (1) the one carried from phase 21 — search **约基别** on the wheel, confirm the row reads `记有约基别` and not "no results", tap it, and confirm the sheet for *Moses is born* prints the People heading with 约基别 and 暗兰 under it, opening `PersonDetailSheet` in the shipped release build; and (2) this iteration's — open the Atlas, switch on **保罗押解往罗马的航程**, and confirm the panel prints the walking band **and** the sea refusal together rather than the band alone, then hover the ruler footer and confirm the ORBIS basis tooltip appears. **A six-line addition to a panel is a layout judgement and the suite only proves it does not throw.** **Prod untouched and unasked-for**, still 1.6.136. |
| Suite | **3,717 tests, exit 0** (+23 this iteration — 22 in the new `test/travel_time_test.dart`, 2 appended to `atlas_page_test.dart`, less the 4 assertions in the one `place_geo_test.dart` block that pinned the deleted `daysOnFootFor`). **The split is deliberate and the two halves are blind to different things.** The widget guards render zh-Hans — they can see that the band and the refusal reach the screen and that the narrowest 320 px pane does not overflow, but **Chinese does not inflect**, so an English `1 days on foot` would pass every rendered test in the repo. The plural rule is therefore asserted **against `uiStrings` directly**, along with the three hedges in `travelDaysBasis` and the en dash in the band. The scriptural guards are the ones worth keeping: one asserts the ORBIS band contains Deuteronomy 1:2's eleven days, and the other asserts the **rejected** convention's 7–8 does not. |
| CI | **green — `success` on `b777508` (the tip) and on `b188a0d`, read back from `gh run list` after both runs completed**, not inferred from the local macOS suite. **The distinction has teeth this iteration for a specific reason: this change is full of non-ASCII.** The band uses an **en dash** (`9–13`, `20–30`), the Chinese strings gained several new clauses, and two widget guards assert on literal zh-Hans substrings (`步行 55–82 天`, `水路 2803 公里`) — encoding or font-fallback differences between hosts are exactly the class of thing that passes locally and reds in CI. It did not. **The local run is the VM on macOS; the app ships as JS**, and the only arithmetic added is `ceil` over a `double` at values in the tens to low thousands, nowhere near an integer boundary, so no `--platform chrome` re-probe was warranted. |
