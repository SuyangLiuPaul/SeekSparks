...
Last updated: 2026-08-31 (twenty-fifth entry)
...
| | |
|---|---|
| `pubspec` / dev | **1.6.188** — search results now mark the Strong's hit in their snippets (`98ee004`/`a077c47`), deployed since this table was last written; not re-audited this run beyond confirming the version and last-deploy commit match the loop's own record. |
...
| `main` | **f7778d2** — #318 residue: the featured chronology wheel's canvas text (ribbon names, spoke titles, verse refs, count badges, both measure helpers) had no CJK fallback, so Chinese drew blank on the web build in the app's default (zh-Hans) locale. `TextPainter` inherits no theme, unlike the `Text` widgets #316 already fixed. Added `canvasTextStyle()` to `font_catalog.dart` (color/fontSize/fontWeight, always carrying `kCjkFontFallback`) and converted the wheel's 8 inline canvas `TextStyle(` sites to it; the 51 theme-inherited `Text`-widget styles on the same page were left untouched. New source-ratchet `test/canvas_cjk_fallback_ratchet_test.dart`: no `TextPainter(` may have a bare `TextStyle(` within its next 15 lines, plus a count pin at exactly 8 `canvasTextStyle(` sites — confirmed failing on the unmodified tree (lines 369/382/2262/2510) before the fix. One bug found in the test itself, not the page: `'canvasTextStyle('` contains the literal substring `'TextStyle('`, so the naive check fired on the very sites meant to pass; fixed by stripping converted sites from the window first. `chronology_page.dart` (3 painters, 0 fallbacks), `bible_timeline_page.dart` (1, 0) and `place_map.dart` (2, 3) are the same defect class, measured and deliberately deferred. +4 tests, 3,764 green, analyze exit 0. NOT deployed — counter 1 of 3. |
| `main` (previous) | **fce19c7** — #315 residue, second pass: closed the last deferred field. `word_study_style.dart`'s dense-column `translit`/`micro` resolved to 9.0px at the default Menu Size (`type.chrome - 2`, and `chrome` IS `WbMetrics.smallPrintFloor`), a fourth shape neither ratchet regex can see — a named-argument initialiser on a data class, read six files away in `originals_sheet.dart` as `fontSize: _st.translit`. Changed both to `type.chrome`. +4 tests, 3,751 green, analyze exit 0. NOT deployed at the time — counter 2 of 3; the counter has since reset via the `98ee004`/`a077c47` dev deploy. |
| `main` (before that) | **f3cac03** — #315 residue, landed the killed prior run's small-print-floor repair: all 52 `fontSize: t.chrome - N` sites (15 files) raised to `t.chrome` = `WbMetrics.smallPrintFloor`, plus a new ratchet forbidding the notation with an 18-entry allowlist for above-floor additive offsets. +1 test, 3,747 green, analyze exit 0. |
| `main` (earlier) | **230f91e** — #317: the ruler's tooltip cited `kBandOnFoot`'s fixed sentence regardless of which band the days above it were computed at; now calls `travelBandBasis()`/`travelBandNotOurs()` for the reader's own band, matching the journey panel. +4 tests (19 in `places_pane_test.dart`), 3,746 green, analyze exit 0. Deployed to dev v1.6.186. |
| deploy | **Skipped this iteration — counter 1 of 3.** Last dev deploy was `a077c47` (v1.6.188, search snippets mark the Strong's hit). `./tools/release_web.sh` not run; no `pubspec.yaml` bump. |
| Suite | **Full run: 3,764 passed, 0 failed** (+4 for the new canvas-CJK-fallback ratchet group; no independent full-suite baseline was measured before this run's edits, so no "was N" delta is claimed for the rest). `flutter analyze` exit 0. |
