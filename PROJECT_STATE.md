...
Last updated: 2026-09-01 (thirty-first entry)
...
| | |
|---|---|
| `pubspec` / dev | **1.6.196** — #317: landed the prior killed run's audited marker-count diff (each journey row now prints `N 站 · N 个标记` beside its range, sourced from the same `markers` list `place_map.dart` paints) and repaired one false sentence inside it. `journeysKey`'s legend, on screen under the toggles whenever any route is switched on, had claimed the marker count is "always fewer" than the stop count — false for `paul-rome`, whose 2 asides take hollow markers without being stops, so its own row prints `14 站 · 16 个标记` one line above the sentence denying it. Rewrote the tail of `journeysKey` in all 3 locales to name all four reasons the counts differ (repeat visit → 1 marker; collapsed run → 1 shared marker; unplaceable stop → 0 markers; aside → +1 marker with no stop) and claim no direction. +2 tests in `journey_asset_test.dart` (one pins that `paul-rome` draws MORE markers than stops, one asserts the legend never says "fewer"/"少于"/"少於"), +1 assertion in the existing `atlas_page_test.dart` widget test. 3,844 green (was 3,840), analyze exit 0. Deployed to dev, counter reset. Cut at `1b2b1bc`/`37ad1f5`. |
| `pubspec` / dev (previous) | **1.6.195** — the reading pane's own headings and background notes, disclosed. `assets/section_titles.json` ships 1,443 app-written headings per title set (cuv/cuv-tr/english-classic), every one carrying an app-written background blurb, rendered by default (`showSectionTitles` defaults true) in the exact register a printed edition's own 段落標題 occupies — and nothing parsed the asset's own `_meta` saying so. Third instance of the class #318 phase 24 and #292 each fixed once. Added a trilingual `note` to `_meta` (insertion only), `SectionTitleMeta`/`provenanceNote()` in the service, a line inside the already-collapsed context panel, an About-page attribution row, and an extended settings subtitle. +5 tests. Full suite 3,823 green (was 3,818), analyze exit 0. Deployed to dev at counter 3 of 3. Cut at `a48f518`/`18df2d6`. |
| `pubspec` / dev (previous) | **1.6.194** — `db79c5e`, prod+dev release (one app name on the splash, a CI floor CI can meet), shipped 2026-08-31 by a concurrent session outside this loop; corrected here from the stale 1.6.191 this row read. This restarted the deploy counter below. |
| `pubspec` / dev (older) | **1.6.186** — #317: the atlas ruler's tooltip always cited `kBandOnFoot`'s fixed "20–30 km a day" sentence under a figure computed at whichever band the reader had actually chosen — a source vouching for an arithmetic it did not make. `travelBandBasis()` already existed for the journey panel; the ruler now calls it too, and appends `travelBandNotOurs()` on a non-default band. Added one visible footer line (`travelBandLabel()`, key `places-map-band`) for the non-default case, since the band picker lives in the journey panel and a tooltip is a hover — unreachable on a tablet, and unreachable at all once the panel is closed while `travelBand` still reaches the map unconditionally. +4 tests (19 in the file), 3,746 green, analyze exit 0. Deployed to dev at counter 3 of 3. Cut at `230f91e`/`1569a8a`. |
...
| `main` | **1b2b1bc** — #317: the legend fix described in the `pubspec`/dev row above. Version bump to v1.6.196 cut at `37ad1f5`. |
| `main` (previous) | **536f866** — #317: landed a killed prior run's audited diff. `atlas_page.dart`'s wilderness itinerary walked `ResolvedJourney.stops` — only what the map can DRAW — so the panel's header said 42 stations while its own list showed 40; Pi-hahiroth (Num 33:7) and Hor-haggidgad (Num 33:32) reached no row at all. New `UnplacedStop`/`itineraryRows` in `journey_route.dart` (parallel list, `stops` untouched so the painter/markers/corridor code that dereferences `place.lat!` stays safe), a new `_unplacedStop` row (dashed badge, verse chip, `journeyNoLocationTag` in all 3 locales) in `atlas_page.dart`. This run's own contribution was the landing: verified the tree against the plan's `[measured]` values, added one widget test proving both camps render as real rows on the real `AtlasPage` in zh-Hans carrying their verses, ran the full suite and analyzer, committed and pushed. +10 tests, 3,840 green (was 3,830), analyze exit 0. NOT deployed — plan-mandated counter 2 of 3. |
| `main` (previous) | **be9ec0d** — #304 check 51 (the plan that produced this run mislabeled it "check 50"; that number was already taken by `2c45a9e`, below, which its own brief did not surface — corrected here, the commit trailer stays as pushed): `assets/book_introductions.json`'s `_meta` carried no `source` at all, and nothing in `lib/` parsed it, so the 66 book-intro cards printed an AUTHOR and a DATE for every book as simply true, rendered by default at the top of chapter 1 — the fourth instance of the disclosure class #318 phase 24, #292 and the section-titles pass each fixed once, and the sharpest of the four since it is the only one printing dates and authorship. `bible_timeline.json` already gives every one of its 98 events a `basis` and an `approximate` flag; measured this run, the two assets restate the same event 11 times, agree on 10 (Exodus 1446, Deuteronomy 1406, 2 Samuel 1010, 1 Kings 970, 2 Kings 586, Malachi 430, Jeremiah 627, Nehemiah 445, Ezra 458, Revelation 95) and disagree on Jonah (timeline −780 vs intro "ca. 760 BCE") — both sides hedged, so by this repo's rule neither number changed; the disagreement is pinned by a test instead of forgotten. `_meta` gained `source`/`notFromAnyEdition`/`datingSystem`/a trilingual `note` (insertion only); new `BookIntroMeta`/`provenanceNote()` in `book_intro_service.dart`; one line under a hairline in the card's expanded state; an About-page attribution row; extended settings subtitle. +7 tests. Full suite 3,830 green (was 3,823), analyze exit 0. NOT deployed — plan-mandated counter 1 of 3. |
| `main` (previous) | **a48f518** — the reading pane's own headings, disclosed: `assets/section_titles.json`'s 1,443 app-written headings per title set (all three sets) each carry an app-written background blurb, rendered by default in the reading pane in the register a printed edition's own 段落標題 occupies, and nothing parsed the asset's `_meta` saying they are original to the app and not from any published edition. Third instance of the class #318 phase 24 and #292 each fixed once. Trilingual `note` added to `_meta` (insertion only, byte-identical otherwise); new `SectionTitleMeta`/`provenanceNote()` in `section_title_service.dart`; one line inside the reading pane's already-collapsed context panel; an About-page attribution row; extended settings subtitle. Deliberately deferred, same unparsed-header shape, measured this run: `book_introductions.json` (closed above), `bible_evidence.json`, `daily_verses.json`, `gospel_synopsis.json`, `chronology.json`, `bible_timeline.json`, `family_tree.json`, `bible_journeys.json` — each needs its own surface, not a sweep. +5 tests. Full suite 3,823 green (was 3,818), analyze exit 0. Deployed to dev at counter 3 of 3, `18df2d6` bumped to v1.6.195. |
| `main` (previous) | **47c7c1c** — #292 disclosure pass: `hebrew_kings.json`'s `_meta.note` suffered the identical gap phase 24 (row below) had just fixed one Resource over, only worse — the chart's only citation surface, `_DetailPanel`'s empty-state Sources block, disappears the instant a king is selected (the page's whole purpose) and never renders at all below `_sideBySideMinWidth` (900px). Split `_meta.note` into a build-only `yearSign` and a trilingual `note` (why Thiele's paired years, e.g. 792/791, collapse to one on this chart); new `HebrewKingsMeta` mirrors `WheelHistoryMeta` exactly, wired through `fromJson` as a `required` field. New "About this chart" AppBar action, always reachable. Reused `kingsSources` verbatim and deliberately did NOT render `_meta.systems` a second time — `kingsSystemsDiffer` already states that caveat permanently in the header bar. `_DetailPanel`'s Sources block left untouched. +6 tests (2 pure, 2 widget at 880×700 proving the About action reaches the citation where the panel cannot, +1 locale-coverage extension); test 1 confirmed to fail to compile on `data.meta` before the service edit. Record counts unmoved (42/4/42/3). Full suite 3,818 green, analyze exit 0. NOT deployed — plan-mandated counter 2 of 3. |
| `main` (previous) | **b2a2c50** — #318 phase 24: `wheel_history.json`'s `_meta` carried eight fields and `WheelHistoryData` parsed none — the chart's own provenance (that no date came from a copyrighted chart) and two statements of absence (Lot's line stopping at Gen 11, the axis stopping at 4000 BC) shipped unreachable, and the disclosure test written for exactly this class iterates the four record lists, so the header was outside its reach by construction. Promoted `provenance`/`coverage`/`axis` to trilingual maps (`scope` already was), added `WheelHistoryMeta`, wired it through `fromJson` and the `load()` merge literal (now `required`, so a dropped wire is a compile error, not a silent loss — confirmed: omitting it before the fix failed to compile), and surfaced it from a new "About this chart" AppBar action, resolving `WbType.of` inside the sheet's own `FutureBuilder` builder rather than the tap handler. +4 tests in a new `wheel_history_meta_disclosure_test.dart`, +1 in the existing disclosure test, +1 widget test. Full suite 3,814 green, analyze exit 0. NOT deployed — plan-mandated counter 1 of 3 (`db79c5e` released v1.6.194 prod+dev on 2026-08-31 outside this loop and restarted the count). |
...
| deploy | **Deployed to dev, v1.6.196 — counter reset.** Plan-mandated `DEPLOY: dev` (landing the killed run's diff plus its own fix). `./tools/release_web.sh` (no flags), verified live output named v1.6.196. |
| Suite | **Full run: 3,844 passed, 0 failed.** `flutter analyze` exit 0, run last after the final edit. |
...
| #292 | Kings of Judah + Israel, synchronised, as a Resource | **built and reachable (corrected 2026-08-26); the disclosure pass shipped 2026-08-31.** `assets/hebrew_kings.json` holds **42 kings and 4 epochs** with `_meta.sources` citing **Thiele, *The Mysterious Numbers of the Hebrew Kings*, 3rd ed. (1983)**, **McFall, *BibSac* 148 (1991): 3–45** for the co-regency years, and the Wikipedia Thiele columns; `lib/pages/hebrew_kings_page.dart` pushed from `workbench_page.dart:593`. The citation was previously reachable only via `_DetailPanel`'s empty-state Sources block, which vanishes the instant a king is selected and never renders below 900px — a new "About this chart" AppBar action now surfaces the sources and the paired-year note (`_meta.note`, trilingual, why 792/791 collapses to one year) unconditionally. Full detail in the `main` row above. **The stated blocker is resolved — whether the ticket CLOSES is an owner call and this does not make it.** |
...
| #304 | Systematic data-integrity audit — "accuracy is the most critical thing" | open, recurring — check 51 landed 2026-08-31 (book-intro dating/authorship provenance) |
...
**Blocked on the owner, four:** #293 (audio
