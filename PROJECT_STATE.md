...
| | |
|---|---|
| `pubspec` / dev | **1.6.191** — prod+dev release, bracketed copy marks / red mark / support@ contact (`cde2420`). This row read 1.6.189 for a stretch after a concurrent session shipped 1.6.190 and 1.6.191 to prod without this file being updated in step; corrected here. This is now the reference point for the deploy counter below. |
...
| deploy | **Skipped this iteration — plan-mandated counter 2 of 3.** Last deploy was `cde2420` (v1.6.191, prod+dev). Commits since then, undeployed: `a47e83b` (KWIC tally, this run), `2220cb0` (a concurrent session's marking fix), `cf2fc76` (font guard, this run). `./tools/release_web.sh` not run; no `pubspec.yaml` bump. |
| Suite | **Full run: 3,799 passed, 0 failed.** `flutter analyze` exit 0, run last after the final edit. |
