...
| deploy | **Skipped this iteration — plan-mandated counter 1 of 3.** Last deploy was `41ccde4` (v1.6.192, prod+dev, built from clean `2220cb0`); it carried everything up to that point, so the deploy counter restarted here. `./tools/release_web.sh` not run; no `pubspec.yaml` bump. |
| Suite | **Full run: 3,802 passed, 0 failed.** `flutter analyze` exit 0, run last after the final edit. |
