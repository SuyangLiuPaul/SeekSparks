...
///
/// The wheel holds 911 records and draws 64 labels at rest, so the
...
///  * THE BARE WILDCARD reaches 688 + 119 + 82 + 22 = 911 records. Not a
...
///  * THE INDEX BAR. Every one of the 911 records, in all three
...
void main() {
  ...
  group('wheelMatches', () {
    ...
    /// The matcher is called once per field per record — 911 records
  ...
  group('every kind of record is reachable', () {
    ...
    /// most of this file.
    test('the bare wildcard returns all 911 records, of all four kinds', () {
      final r = find('*');
      expect(r.hits, hasLength(911));
      final byKind = {
  ...
  /// reading it in.
  ///
  /// So this group asks the corpus itself: 911 records x 3 locales,
  ...
  group('the index bar — every record answers to its own name', () {
    ...
    test('all 911, in all three locales, come back when typed', () {
      for (final locale in _locales) {
        final ranks = ownTitleRank(locale);
        expect(ranks, hasLength(911),
    ...
    /// answering to itself. That is all 911 of them, so the sweep
    ...
    test('the sweep is not passing vacuously', () {
      ...
      expect(titles.where((t) => RegExp(r'[A-Z]').hasMatch(t)), hasLength(911),
