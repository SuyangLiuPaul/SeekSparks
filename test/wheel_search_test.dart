...
///
/// The wheel holds 1005 records and draws 64 labels at rest, so the
...
///  * THE BARE WILDCARD reaches 707 + 152 + 82 + 22 + 42 = 1005 records. Not a
...
///  * THE INDEX BAR. Every one of the 1005 records, in all three
...
void main() {
  ...
  group('the printed year and the read year are the same year', () {
    ...
    test('every event round-trips through both functions, in every locale',
        () {
      expect(data.events, hasLength(707));
      final broken = <String>[];
  ...
  group('wheelMatches', () {
    ...
    /// The matcher is called once per field per record — 1005 records
  ...
  group('every kind of record is reachable', () {
    ...
    /// most of this file.
    test('the bare wildcard returns all 1005 records, of all four kinds', () {
      final r = find('*');
      expect(r.hits, hasLength(1005));
      final byKind = {
  ...
  group('the stream filter marks results, it never swallows them', () {
    ...
    test('hiding every band still returns everything, all flagged', () {
      ...
      expect(all.hits, hasLength(1005));
  ...
  /// reading it in.
  ///
  /// So this group asks the corpus itself: 1005 records x 3 locales,
  ...
  group('the index bar — every record answers to its own name', () {
    ...
    test('all 1005, in all three locales, come back when typed', () {
      for (final locale in _locales) {
        final ranks = ownTitleRank(locale);
        expect(ranks, hasLength(1005),
    ...
    /// answering to itself. That is all 1005 of them, so the sweep
    ...
    test('the sweep is not passing vacuously', () {
      ...
      expect(titles.where((t) => RegExp(r'[A-Z]').hasMatch(t)), hasLength(1005),
