...
///
/// The wheel holds 990 records and draws 64 labels at rest, so the
...
///  * THE BARE WILDCARD reaches 698 + 149 + 82 + 22 + 39 = 990 records. Not a
...
///  * THE INDEX BAR. Every one of the 990 records, in all three
...
void main() {
  ...
  group('the printed year and the read year are the same year', () {
    ...
    test('every event round-trips through both functions, in every locale',
        () {
      expect(data.events, hasLength(698));
      final broken = <String>[];
    ...
    test('every power span round-trips too', () {
      expect(data.powers, hasLength(149));
      final broken = <String>[];
  ...
  group('wheelMatches', () {
    ...
    /// The matcher is called once per field per record — 990 records
  ...
  group('every kind of record is reachable', () {
    ...
    /// most of this file.
    test('the bare wildcard returns all 990 records, of all four kinds', () {
      final r = find('*');
      expect(r.hits, hasLength(990));
      final byKind = {
  ...
  /// reading it in.
  ///
  /// So this group asks the corpus itself: 990 records x 3 locales,
  ...
  group('the index bar — every record answers to its own name', () {
    ...
    test('all 990, in all three locales, come back when typed', () {
      for (final locale in _locales) {
        final ranks = ownTitleRank(locale);
        expect(ranks, hasLength(990),
    ...
    /// answering to itself. That is all 990 of them, so the sweep
    ...
    test('the sweep is not passing vacuously', () {
      ...
      expect(titles.where((t) => RegExp(r'[A-Z]').hasMatch(t)), hasLength(990),
