/// 2026-08 (ported from YsWords v1.4.0): regression tests for ImportService, the
...
void main() {
  ...
  group('ImportService.parse — schema validation', () {
    ...
    // SeekSparks-specific: ExportService still writes the inherited
    // `yswords-export` schema id, so that one MUST keep working or every
    // backup a user already holds becomes unimportable. `seeksparks-export`
    // is accepted too so renaming the export id later is non-breaking.
    test('both the inherited and the SeekSparks schema ids are accepted', () {
      for (final id in ['yswords-export', 'seeksparks-export']) {
        final parsed = ImportService.parse(
            '{"schema": "$id", "schemaVersion": 1, '
            '"highlights": {"Genesis-1-1": 42}, "bookmarks": [], "notes": {}}');
        expect(parsed.highlights, {'Genesis-1-1': 42},
            reason: '$id must be importable');
      }
    });

    test('a Markdown export is rejected with a format-specific message', () {
      expect(
        () => ImportService.parse('# SeekSparks export\n\nsome prose'),
        throwsA(isA<FormatException>().having(
            (e) => e.message, 'message', contains('Markdown'))),
      );
    });
  });
