...
//
// SeekSparks v1 intentionally ships with NO Firebase project — it's a
// separate app from YsWords with no shared account, so it needs its
// own project rather than reusing YsWords' (sync data paths have no
// per-app namespace, so sharing would let a user signed into both
// apps silently overwrite each other's highlights/notes). Cloud sync
// is a deferred phase; when it's wanted, follow the numbered setup
// below with a fresh project.
//
// One-time setup (~15 minutes):
// ──────────────────────────────────────────────────────────────────
// 1. Go to https://console.firebase.google.com and click
//    "Add project". Name it whatever you like (e.g. "seeksparks").
...
//      Add your Netlify site domain (e.g. `seeksparks.netlify.app`)
