// 2026-05-18 (v1.2.53): extracted from `lib/services/fetch_books.dart`.
// The map originally lived there alongside `FetchBooks.execute`, but
// that file imports `MainProvider`, which transitively imports
// `cloud_sync_service.dart`'s `dart:js_interop` — fine on web but a
// hard compile error on the unit-test VM. Pulling the const Map into
// its own dependency-free file lets test-only services (e.g.
// `LebInsightsService`) consume it without dragging in the Firebase
// / cloud-sync chain.
//
...
// `fetch_books.dart` for the same reason `bookNameToEnglish` did —
// `buildBooksFromVerses` needs it and must not drag `MainProvider`
// into the unit-test VM. `fetch_books.dart` re-exports it.
