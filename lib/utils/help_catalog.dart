...
/// 怎么用 … 不知道可以在那里找 而且需要有search功能找相关功能」, then
/// 「要详细」「功能和shortcut之类以及介绍」「不同platform之类的」.
///
...
///    not exist.
///  * **Shortcuts.** Not written here at all. The page prints them from
///    the tables in `keyboard_shortcuts.dart` that the handlers dispatch
///    from, so a key on the Help page is a key that works.
///  * **Search.** Every language at once: a reader in the Chinese
///    interface who types `shortcut`, or `F2`, or 繁體 投影 in the
///    simplified one, still finds it. See [searchHelp].
...
const Map<HelpPlatform, HelpText> kHelpPlatformNames = {
  ...
  HelpPlatform.ios: HelpText('iPad', 'iPad', 'iPad'),
  HelpPlatform.android: HelpText('Android 平板', 'Android 平板', 'Android tablet'),
...
const Map<HelpSection, HelpText> kHelpSectionNames = {
  ...
  HelpSection.reading: HelpText('阅读与对照', '閱讀與對照', 'Reading & comparing'),
  HelpSection.search: HelpText('命令行与搜索', '命令列與搜尋', 'Command line & search'),
  HelpSection.analysis: HelpText('分析窗', '分析窗', 'Analysis window'),
  HelpSection.tools: HelpText('工具', '工具', 'Tools'),
  HelpSection.resources: HelpText('资料库', '資料庫', 'Resources'),
  HelpSection.personal: HelpText('笔记、高亮与数据', '筆記、螢光標記與資料', 'Notes, highlights & your data'),
  HelpSection.settings: HelpText('设置与更新', '設定與更新', 'Settings & updates'),
  HelpSection.platforms: HelpText('各平台', '各平台', 'Platforms'),
  HelpSection.shortcuts: HelpText('快捷键', '快捷鍵', 'Keyboard shortcuts'),
};

/// Somewhere the Help page can take the reader. Resolved by the page (a
/// route) or by the workbench (an action on its own state); the switch
/// over this enum is exhaustive in both, so a new door does not compile
...
class HelpTopic {
  ...
  /// Lines of the command-line grammar this topic teaches, as
  /// `ui_strings` keys — the same keys the command line's own `?` card
  /// prints, so the two cannot teach different syntax.
...
/// stored on it — the page passes each shortcut's label here, so `F4`
/// finds the passage report.
List<HelpHit> searchHelp(
  ...
  for (final t in topics) {
    ...
    final prose = [
      ...
      for (final k in t.examples) ...?uiStrings[k]?.values,
    ].map(normalizeHelpQuery).join('\n');
