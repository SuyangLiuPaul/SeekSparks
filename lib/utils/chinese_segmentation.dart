...
/// (`search_synonyms.dart`, `bible_names.json`, the Strong's alias
/// table). Everything the vocabulary does not cover falls back to one
...
/// (`command_query.dart`'s doc states the rule, and it is deliberate),
/// so 爱神 finds 爱神 without anybody segmenting anything. Segmentation
...
///
/// Deliberately duplicates `related_verses.dart`'s `isCjkChar`, ranges
/// and all, instead of importing it. That file is 685 lines of
/// SeekSparks relatedness scoring, and this layer is going into the
/// YsWords repository unchanged; a four-line predicate is a cheaper
/// thing to carry than a dependency on a file that does not belong
/// there. `test/chinese_segmentation_test.dart` asserts the two agree
/// over every code point in the ranges, so the duplication cannot rot
/// into a disagreement.
