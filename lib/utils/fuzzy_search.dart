/// 2026-09-08 (SeekSparks): the looser reading of a query that the
/// reader has to ask for by name.
///
/// ## The defect, measured
///
/// Every count below is over the real `searchCorpusKey` of a shipped
/// edition — what a search actually compares against, which is not the
/// same thing as the asset text.
///
/// This app restores the divine name, and `text_patterns.dart` does it
/// in the INDEX: 耶和华 becomes 雅伟 and all-caps `LORD` becomes
/// `Yahweh` as the corpus key is built. Nothing does the same to the
/// query, so the spelling every Chinese Bible in print uses reaches
/// nothing at all:
///
///     typed      cuvs-yhwh  cuvs-yhwh-tr  cuvs-plus    kjv
///     雅伟           6,102             0      5,908      —
///     雅偉               0         6,102          0      —
///     耶和华             0             0          0      —
///     yahweh         —             —          —     5,614
...
/// finds 9 verses in `cuvs-yhwh-tr` and 0 in `cuvs-yhwh`; 愛 finds 822
/// and 0.
///
...
/// l-o-v-e in a row — and `loved` reaches only the 198 verses that
/// spell it that way, never the 546 that say "love".
///
/// ## Why it is OFF by default, and why that is not timidity
///
/// This app's whole claim is that its search is exact and auditable —
/// `plain_search.dart` exists because a match that stepped over a word
/// gap was unexplainable, and `search_broadening.dart` exists because
/// an offer the app had not measured was a dead end dressed as help.
/// A search that silently widened itself would undo both. So:
///
///   * [fuzzySearchEnabled] is **false** until something sets it, and
///     the setter is the only way in. The precedent is
///     `search_folding.dart`, whose own comment argues the case for a
///     single global over a flag threaded through six call sites: the
///     failure mode of threading is a *silent asymmetry*, where one
///     side of a comparison is broadened and the other is not, and the
///     search then finds nothing at all.
///   * Every broadened row can say so. [plainSearchMatchKind] answers
///     "was this hit literal?" for one verse in the cost of one match,
///     so a result list can mark the rows that only the looser reading
///     found. `strip_chronology_layout.dart` states the rule this
///     obeys — "nothing narrows in silence" — and a view that WIDENS in
///     silence is the same defect facing the other way.
///
...
/// the reader typed:
///
///   1. [FuzzyMatch.literal] — exactly what `plain_search.dart` matched
...
///      `loved` goes from 198 KJV verses to 548. `flies` does NOT reach
...
///      and its segments occur together in 4.
///
/// ## What this does NOT do
///
///   * **It does not touch the command grammar.** `.love god`, `'in the
///     beginning` and `G25 AND G26` are unaffected: `command_query.dart`
///     is a word-level matcher with its own tokenizer, and its rule
///     that "a Chinese term is a substring search and an English term is
///     a word search" is deliberate. Broadening it is a separate piece
...
/// `chinese_segmentation.dart` and `search_synonyms.dart` are going into
/// the YsWords repository unchanged. Nothing here imports Flutter, an
...
/// actually moved.
///
/// Shaped exactly like `setSearchIgnoresPointing`, and for the same
/// reason: one writer, one global, no half-applied state. The intended
/// writer is `AppSettings` — a persisted preference with a Settings
/// toggle beside "Ignore vowel points and accents", read on load and on
/// change — and it is not wired there yet because that file is under
/// another agent's hand this week.
...
///
/// Only the 1,101 characters the shipped 和合本 actually spells
...
///
/// The KJV's vocabulary is about 13,000 words and the BSB's about
/// 16,000, so one edition fits with room to spare; the cap is there so
