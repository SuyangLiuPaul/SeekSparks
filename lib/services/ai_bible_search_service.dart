...
class AiBibleSearchService {
  ...
  static Future<AiBibleSearchResult> ask({
    ...
    var transientReason = 'SeekSparks AI is busy right now. Please try again.';
    for (var attempt = 0; attempt < backoffs.length; attempt++) {
      ...
      try {
        ...
        transientReason =
            'SeekSparks search took too long. Please try again or rephrase.';
        continue; // transient → retry
      } catch (_) {
        transientReason = 'SeekSparks search is not available right now.';
        continue; // transient → retry
      }
      ...
      if (code == 200) {
        try {
          ...
          return AiBibleSearchResult.unavailable(
              'SeekSparks search returned an unexpected response.');
        }
      }
      if (code == 404) {
        return AiBibleSearchResult.unavailable(
            'SeekSparks search is not available yet (function not deployed).');
      }
      if (code == 429) {
        return AiBibleSearchResult.unavailable(
          serverError() ??
      ...
      return AiBibleSearchResult.unavailable(
        serverError() ?? 'SeekSparks search returned $code.',
      );
    }
