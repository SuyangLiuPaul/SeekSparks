...
///
/// 2026-08 (ported from YsWords v1.4.0): added in response to "export
/// exists, but there's no way to bring that data back in." Only the JSON
/// format is importable — `ExportService`'s own doc comment already says
/// Markdown is prose for pasting into other apps, not reliably
/// re-parseable; JSON is the one "designed to be re-importable" format.
///
...
class ImportService {
  /// Schema ids accepted on import.
  ///
  /// `ExportService.toJson` writes `yswords-export` — SeekSparks inherited
  /// that id from the YsWords fork and it was deliberately never renamed,
  /// because the two apps store highlights/bookmarks/notes in exactly the
  /// same shape. Keeping it means (a) every export file already in a
  /// user's hands stays importable, and (b) data moves freely between
  /// SeekSparks and YsWords. `seeksparks-export` is accepted too so that
  /// renaming the export id later is a non-breaking change.
  static const _schemas = {'yswords-export', 'seeksparks-export'};
  static const _schemaVersion = 1;

  /// Parses [raw] as a SeekSparks (or YsWords) export JSON string.
  ///
  /// Throws [FormatException] when:
  ///   - [raw] isn't valid JSON at all (propagated from `jsonDecode`).
  ///   - the decoded value isn't a JSON object.
  ///   - `schema` isn't a recognized export id — this isn't one of our
  ///     export files at all.
  ...
  static ParsedImport parse(String raw) {
    ...
    try {
      ...
      // Only JSON round-trips — the Markdown export is prose for pasting
      // into OTHER apps (Notion/Obsidian/etc.), not structured data, so
      // there's nothing here to parse it back into. Both apps' Markdown
      // exports carry a recognizable header (see ExportService.toMarkdown),
      // so catch this specific mix-up with a helpful message instead of a
      // bare JSON-syntax error that would leave a user wondering why their
      // own export doesn't import.
      final head = raw.trimLeft();
      if (head.startsWith('# SeekSparks export') ||
          head.startsWith('# YsWords export')) {
    ...
    if (decoded is! Map) {
      throw const FormatException('Not a SeekSparks export file.');
    }
    if (!_schemas.contains(decoded['schema'])) {
      throw const FormatException('Not a SeekSparks export file.');
    }
