#!/usr/bin/env bash
# 2026-09-05: cut a GitHub Release, so the in-app update check has
...
# asset to that tag's Release, creating it if absent. All five workflows
# already existed here; only this script was missing.
#
# Ported from YsWords on 2026-09-05, where the identical gap had already
# been found. This repo's version of it was worse: ONE release has ever
# been cut, v1.6.164 on 2026-08-24, and `pubspec.yaml` had reached
# 1.6.236 — seventy-two versions later. `UpdateService` reports an update
# only when the latest TAG is newer than the running `kAppVersion`, so
# for those seventy-two versions the check was not merely quiet, it was
# WRONG: anyone on a build newer than 1.6.164 who asked for an update was
# offered an older one.
#
